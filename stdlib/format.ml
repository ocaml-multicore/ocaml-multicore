...
let buffered_out_string key str ofs len =
  Buffer.add_substring (ThreadLocal.get key) str ofs len

let buffered_out_flush oc key () =
  let buf = ThreadLocal.get key in
...
let std_buf_key = ThreadLocal.new_key (fun () -> Buffer.create pp_buffer_size)
let err_buf_key = ThreadLocal.new_key (fun () -> Buffer.create pp_buffer_size)
...
and open_tag v = pp_open_tag (TLS.get std_formatter_key) v
and close_tag v = pp_close_tag (TLS.get std_formatter_key) v
...
let set_all_formatter_output_functions ~out =
  pp_set_all_formatter_output_functions (TLS.get std_formatter_key) ~out


(* Deprecated : subsumed by get_formatter_out_functions *)
let get_all_formatter_output_functions () =
  pp_get_all_formatter_output_functions (TLS.get std_formatter_key) ()
...
let set_formatter_tag_functions v =
  pp_set_formatter_tag_functions (TLS.get std_formatter_key) v
and get_formatter_tag_functions () =
  pp_get_formatter_tag_functions (TLS.get std_formatter_key) ()
