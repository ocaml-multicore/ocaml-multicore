...
let prng_key = ThreadLocal.new_key Random.State.make_self_init
...
let create ?(random = !randomized) initial_size =
  ...
  let seed =
    if random then Random.State.bits (ThreadLocal.get prng_key) else 0
  in
...
let rebuild ?(random = !randomized) h =
  ...
  let seed =
    if random then Random.State.bits (ThreadLocal.get prng_key)
