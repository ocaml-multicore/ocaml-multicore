...
let last_search_result_key = ThreadLocal.new_key (fun () -> [||])

let string_match re s pos =
  let res = re_string_match re s pos in
  ThreadLocal.set last_search_result_key res;
  Array.length res > 0
...
let group_beginning n =
  let last_search_result = ThreadLocal.get last_search_result_key in
  let n2 = n + n in
