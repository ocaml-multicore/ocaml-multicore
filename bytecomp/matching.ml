...
let ctx_matcher p =
  ...
  match p.pat_desc with
  | Tpat_construct (_, _, cstr,omegas) ->
      begin match cstr.cstr_tag with
      | Cstr_extension _ ->
          let nargs = List.length omegas in
          (fun q rem -> match q.pat_desc with
          | Tpat_construct (_, _, cstr',args)
...
let rec exc_inside p = match p.pat_desc with
  | Tpat_construct (_, _, {cstr_tag=Cstr_extension _},_) -> true
  | Tpat_any|Tpat_constant _|Tpat_var _
  | Tpat_construct (_,_,_,[])
  | Tpat_variant (_,None,_)
    -> false
  | Tpat_construct (_,_,_,ps)
  | Tpat_tuple ps
  | Tpat_array ps
      -> exc_insides ps
...
let pat_as_constr = function
  | {pat_desc=Tpat_construct (_, _, cstr,_)} -> cstr
...
and split_constr cls args def k =
  ...
  | Tpat_construct (_,_,{cstr_tag=Cstr_extension _},_) ->
...
let get_key_constr = function
  | {pat_desc=Tpat_construct (_, _, cstr,_)} -> cstr.cstr_tag
  | _ -> assert false

let get_args_constr p rem = match p with
| {pat_desc=Tpat_construct (_, _, _, args)} -> args @ rem
| _ -> assert false
...
| 0 ->
    ...
    | Tpat_construct (_, _, cstr1, []) when cstr.cstr_tag = cstr1.cstr_tag ->
        rem
    | Tpat_any -> rem
    | _ -> raise NoMatch in
    matcher_rec
