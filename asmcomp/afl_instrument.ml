...
let rec with_afl_logging b =
  ...
  let instrumentation =
    ...
    let op oper args = Cop (oper, args) in
    Clet(afl_area, op (Cload Word) [afl_area_ptr],
    Clet(cur_pos,  op Cxor [op (Cload Word) [afl_prev_loc];
                            Cconst_int cur_location],
    Csequence(
      op (Cstore Byte_unsigned)
...
and instrument = function
  ...
  | Cswitch (e, cases, handlers) ->
     Cswitch (instrument e, cases, Array.map with_afl_logging handlers)
  ...
  | Ccatch (nfail, ids, cases, body) ->
     Ccatch (nfail, ids, instrument cases, instrument body)
