...
let rec compat p q =
  ...
  | Tpat_construct (_, _, c1,ps1), Tpat_construct (_, _, c2,ps2) ->
...
let rec pretty_val ppf v =
  ...
  | Tpat_construct (_, _, cstr, []) ->
      fprintf ppf "%s" cstr.cstr_name
  | Tpat_construct (_, _, cstr, [w]) ->
      fprintf ppf "@[<2>%s@ %a@]" cstr.cstr_name pretty_arg w
  | Tpat_construct (_, _, cstr, vs) ->
...
let simple_match p1 p2 =
  ...
  | Tpat_construct(_, _, c1, _), Tpat_construct(_, _, c2, _) ->
...
| Tpat_construct(_, _, cstr, args) -> args
...
| (Tpat_any | Tpat_var(_)) ->
    begin match p1.pat_desc with
      Tpat_construct(_, _, _,args) -> omega_list args
...
let rec normalize_pat q = match q.pat_desc with
  ...
  | Tpat_construct  (lid, total, c,args) ->
      make_pat
        (Tpat_construct (lid, total, c, omega_list args))
...
| {pat_desc = Tpat_record (omegas,closed)} ->
    ...
    make_pat
      ...
      q.pat_type q.pat_env::
    rest
| {pat_desc = Tpat_construct (lid, t, c,omegas)} ->
    let args,rest = read_args omegas r in
    make_pat
      (Tpat_construct (lid, t, c,args))
      q.pat_type q.pat_env::
    rest
...
let generalized_constructor x =
  match x with
    ({pat_desc = Tpat_construct(_,_,c,_);pat_env=env},_) ->
      c.cstr_generalized
  | _ -> assert false
...
| ({pat_desc=Tpat_construct(_,_,{cstr_tag=(Cstr_constant _|Cstr_block _)},_)}
     as p,_) :: _ ->
...
| Some ext -> match env with
  | ({pat_desc =
      Tpat_construct(_,_, {cstr_tag=(Cstr_constant _|Cstr_block _)},_)}
     as p, _) :: _ ->
      let path = get_type_path p.pat_type p.pat_env in
      Path.same path ext
  | _ -> false
...
| ({pat_desc = Tpat_construct (lid, total,
      ({cstr_tag=Cstr_extension _} as c),_)},_) :: _ ->
    let c = {c with cstr_name = "*extension*"} in
      make_pat (Tpat_construct(lid, total, c, [])) Ctype.none Env.empty
| ({pat_desc = Tpat_construct (_,_, _,_)} as p,_) :: _ ->
    ...
    | _ ->
        let get_tag = function
          | {pat_desc = Tpat_construct (_,_,c,_)} -> c.cstr_tag
...
let rec has_instance p = match p.pat_desc with
  ...
  | Tpat_construct (_,_,_,ps) | Tpat_tuple ps | Tpat_array ps ->
      has_instances ps
...
let rec le_pat p q =
  ...
  | Tpat_constant(c1), Tpat_constant(c2) -> const_compare c1 c2 = 0
  | Tpat_construct(_,_,c1,ps), Tpat_construct(_,_,c2,qs) ->
    ...
    let rec loop pat =
      ...
      | Tpat_tuple lst ->
          ...
          List.map
            ...
            results
      | Tpat_construct (cstr_lid, total, cstr,lst) ->
...
(*              Hashtbl.iter (fun s (path, _) ->
        ...
        | Some v ->
            let errmsg =
              match v.pat_desc with
                Tpat_construct (_, _, {cstr_name="*extension*"}, _) ->
...
| Tpat_construct(_, _, {cstr_tag=(Cstr_constant _|Cstr_block _)},ps) ->
    let path =  get_type_path p.pat_type p.pat_env in
    List.fold_left
...
| Tpat_construct (_, _, {cstr_tag=Cstr_extension _}, ps)->
...
| Tpat_any | Tpat_var _ | Tpat_constant _ | Tpat_variant (_, None, _) ->
    true
| Tpat_tuple ps | Tpat_construct (_, _,  _, ps) | Tpat_array ps ->
